...
class JsonPromptDataset(Dataset):
    """Read prompts from a JSON array, extracting text by a configurable key."""
    def __init__(self, json_path, text_key="text"):
        with open(json_path, encoding="utf-8") as f:
            data = json.load(f)
        self.prompts = [item[text_key] for item in data if text_key in item]
